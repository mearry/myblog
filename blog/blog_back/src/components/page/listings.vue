  ...
  <table v-for="(item,ind) in data">
    <thead>
     <tr>
       ...
       <td>文章数量:0</td>
       <td class="last"><button>修改</button><button @click="deleteOne(item.onedata.id,item.onedata.enname,ind)">删除</button></td>
     </tr>
    </thead>
    <tbody>
     <tr v-for="(i,index) in item.twodata">
       ...
       <td class="last"><button>修改</button><button  @click="deletes(item.onedata.id,i.id,item.onedata.enname,index,ind)">删除</button></td>
     </tr>
    </tbody>
  </table>
  </div>
